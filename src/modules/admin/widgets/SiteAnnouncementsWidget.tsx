...
export default function SiteAnnouncementsWidget() {
  ...
  const hasAnnouncements = () => (list()?.length ?? 0) > 0;

  return (
    // Non-admins get nothing to act on when the board is empty, so the
    // widget just doesn't render for them. Admins still see it (with the
    // compose form below) so there's a way to post the first announcement.
    <Show when={!list.loading && (isAdmin() || hasAnnouncements())}>
