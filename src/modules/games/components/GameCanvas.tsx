...
const THEME_VARS = [
  "--color-base", "--color-surface", "--color-elevated", "--color-overlay",
  "--color-txt", "--color-muted", "--color-subtle",
  "--color-rim", "--color-rim-strong",
  "--color-accent", "--color-accent-muted", "--color-accent-txt", "--color-accent-fg",
] as const;

function buildSrcdoc(gameId: string, baseUrl: string): string {
  const s = getComputedStyle(document.documentElement);
  const vars = THEME_VARS.map((v) => `${v}:${s.getPropertyValue(v).trim() || "inherit"}`).join(";");
...
<script src="${baseUrl}puzzles/${gameId}.js"></script>
</body>
</html>`;
}
